...
def load_progress(topic: str) -> dict:
    """Return {question_index: True} for every question marked done."""
    try:
        with _db_connect() as conn:
            rows = conn.execute(
                "SELECT question_index FROM progress WHERE topic = ? AND done = 1",
                (topic,),
            ).fetchall()
...
def save_progress(topic: str, index: int, done: bool) -> None:
    """Persist a single question's checked state immediately."""
    try:
        with _db_connect() as conn:
            if done:
                conn.execute(
                    "INSERT INTO progress (topic, question_index, done) "
                    "VALUES (?, ?, 1) "
                    "ON CONFLICT(topic, question_index) DO UPDATE SET done = 1",
                    (topic, index),
                )
            else:
                conn.execute(
                    "DELETE FROM progress WHERE topic = ? AND question_index = ?",
                    (topic, index),
                )
            conn.commit()
    except Exception as ex:
...
def main(page: ft.Page):
    ...
    # App/window icon (used for the desktop window titlebar/taskbar icon
    # and as the browser favicon when running as a web app). Flet serves
    # anything under assets_dir at the app root, so "logo.png" resolves to
    # assets/logo.png — see the ft.app(..., assets_dir="assets") call at
    # the bottom of this file. (This is kept intentionally — only the
    ...
    def build_question_control(
        ...
        that database whenever the checkbox is tapped.
        """
        code, lang, remaining_text = _parse_code_question(question)

        # Tick-only progress checkbox — no label text, just a checkbox the
        # user taps to mark a question as done. State is persisted by the
        # caller via on_toggle so it survives closing and reopening the app.
        progress_checkbox = ft.Checkbox(
            value=done,
            label="",
    ...
    def show_quiz(topic: str):
        ...
        Per-question "done" progress is loaded from the local SQLite
        database (pyquizy_progress.db) before the view is built, so ticks
        ...
        progress = load_progress(topic)  # {question_index: True}

        progress_text = ft.Text(
            f"Progress: {len(progress)}/{total}",
            size=13,
            weight=ft.FontWeight.BOLD,
        )

        def make_toggle_handler(i: int):
            def handler(e):
                ...
                if checked:
                    ...
                    progress.pop(i, None)
                save_progress(topic, i, checked)
                progress_text.value = f"Progress: {len(progress)}/{total}"
